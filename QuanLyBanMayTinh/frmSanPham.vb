Public Class frmSanPham
    ...
    Public Sub New(IsEdit As Boolean)
        ...
        _isEdit = IsEdit
    End Sub
    'Định nghĩa hàm thêm bản ghi SanPham vào database
    Private Function ThemSanPham() As Boolean
        Dim sqlQuery As String = "INSERT INTO SanPham (Ten,Gia,SoLuong,MaCM)"
        sqlQuery += String.Format("VALUES ('{0}','{1}','{2}','{3}')", txtTenSP.Text, txtGia.Text, txtSL.Text, txtMaCM.Text)
        Return _DBSQL.ExecuteNoneQuery(sqlQuery)
    End Function

    'Dinh nghia ham update
    Private Function SuaSanPham() As Boolean
        Dim sqlQuery As String = String.Format("UPDATE SanPham SET Ten = '{0}',Gia='{1}',SoLuong='{2}' WHERE MaSP='{3}'",
                                               Me.txtTenSP.Text, Me.txtGia.Text, Me.txtSL.Text, Me.txtMaSP.Text)
        Return _DBSQL.ExecuteNoneQuery(sqlQuery)
    End Function

    'Dinh nghia ham kiem tra gia tri truoc khi insert du lieu vao database
    Private Function IsEmpty() As Boolean
        Return String.IsNullOrEmpty(txtTenSP.Text) OrElse String.IsNullOrEmpty(txtGia.Text) OrElse String.IsNullOrEmpty(txtSL.Text) OrElse
        String.IsNullOrEmpty(txtMaCM.Text)
    End Function
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        Else
            If _isEdit Then 'Neu la Edit thi goi ham Update
                If SuaSanPham() Then 'Neu update thanh cong thi thong bao
            ...
            Else            'Neu khong phai Edit thi goi ham Insert
                If ThemSanPham() Then 'Neu Insert thanh cong thi thong bao
